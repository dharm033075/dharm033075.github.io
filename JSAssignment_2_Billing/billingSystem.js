...
var items=[{"itemName":"French Fries","itemPrice":75,"course":"starter"},{"itemName":"Dosa","itemPrice":15,"course":"starter"},{"itemName":"Butter Roti","itemPrice":18,"course":"starter"},
{"itemName":"Paneer Butter Masala","itemPrice":50,"course":"Main Course"},{"itemName":"Hyderabadi Biryani","itemPrice":225,"course":"Main Course"},{"itemName":"Mutton Biryani","itemPrice":250,"course":"starter"},
{"itemName":"Veg Biryani","itemPrice":135,"course":"Main Course"},{"itemName":"Beer","itemPrice":95,"course":"Main Course"},{"itemName":"Sprite","itemPrice":35,"course":"starter"},{"itemName":"Dosa","itemPrice":15,"course":"starter"},{"itemName":"Butter Roti","itemPrice":18,"course":"Main Course"}];
...
function searchItem(){
	...
	var items1=panel.getElementsByClassName('panel-heading');
	for(var i=0;i<items.length;i++){
		if(items[i].itemName.toUpperCase().indexOf(item.trim())>-1 || items[i].course.toUpperCase().indexOf(item.trim())>-1){
			items1[i].parentElement.style.display="";
		}else{
			items1[i].parentElement.style.display="none";
		}
	}
...
function addItemsInBillTable(selectedTab){	
	...
	for(var i=0;i<selectedTab.length;i++){
		...
		billJSON.push({"billItem":selectedTab[i].itemName, "itemPrice":selectedTab[i].price,"Numbers":selectedTab[i].itemCount});
...
function generateBill(event){
	...
	if(selectedTab.length==0){			
			...
			billJSON.push({"totalPrice":sum(tableItself)});
	}
